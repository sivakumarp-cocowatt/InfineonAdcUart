...
uint8 g_txData[] = "ADC_Single_Channel..\n\r";
uint8 g_rxData[SIZE] = {''};
...
{
    IfxAsclin_Asc_write(&g_ascHandle, Locu8_Data, &Locu8_Size, TIME_INFINITE);   /* Transmit data via TX */
}

/* Generic UART send function like printf */
void send_UART(const char *format, ...)
{
    ...
    Ifx_SizeT size = (Ifx_SizeT)strlen(buffer);
    IfxAsclin_Asc_write(&g_ascHandle, buffer, &size, TIME_INFINITE);
}
